...
$(window).load(function () {
    $("#formSubmitReg1").click(function () {
        if (validateForm()) {
            ...
            callApiForRegistration(todo, fname, lname, email, password,"",1);
        }
    });
...
//            alert();
            ...
            var username = "test";
            var password = $('#password').val();
            var avatar = $(".flex-active-slide img").attr("src");
            alert(avatar);
            callApiForRegistration(todo, fname, lname, email, username, password,avatar);
        }
    });
}
